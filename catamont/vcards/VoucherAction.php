...
class VoucherAction
{
	...
	public function generate_voucher($request)
	{
		...
		$LOG_FILE_HANDLER=new Createlog('DEBUG',"vcards_generate_voucher");
		$LOG_FILE_HANDLER->logging('DEBUG',"START generation php ");
		$mysqlObj=new mysqlDatabase(DB_USERNAME,DB_PASSWORD,DATABASE,SERVERIP,$LOG_FILE_HANDLER);
       		$date = date("Y-m-d H:i:s");
        	$icici_select_query="select * from icici_voucher_details where voucher_code='$vcode' and client_id=2 and voucher_expiry_date >'$date'";
	        $icici_voucher_detailsArr=$mysqlObj->getarray($icici_select_query);
        	if(!$icici_voucher_detailsArr[0]['vid'])
	        {
                	$LOG_FILE_HANDLER->logging('ERROR',"Voucher Code not valid  : $icici_select_query");
		...
		$return_msg="";
		if($voucher_status=="assigned" || $voucher_status=="requested")
		{
			if($voucher_status=="assigned")
			{
				$return_msg="<strong>Voucher Code already Activated</strong></br>Flipkart Gift Card Details was already sent on the Email ID entered by you.</br>Flipkart Gift Card Details was already sent on the Mobile Number entered by you.";
			...
			elseif($voucher_status=="requested")
			{
				$return_msg="<strong>Dear Customer we have already received your request. Your Flipkart Gift Voucher will be processed and sent to your registered Email Id and Mobile Number within the next 48 hours.</strong>";
				$LOG_FILE_HANDLER->logging('ERROR',"Request has been taken We will send you Flipkart Gift card details on the requested Email id and Mobile Number : $icici_select_query");		
			}
                        return array("status"=>false,"errorMessage"=>"$return_msg");
		}
		$select_query="select * from voucher_details where client_id=2 and status='created' and voucher_price=$icici_vcode_price limit 1";
                $card_count=$mysqlObj->getcount($select_query);
		if($card_count<=0)
		{
			if($mobileno)
                	{
				$SMS_TEMPLATE="Dear Customer The request for your Flipkart Gift Voucher is registered and will be processed within the next 48 hours.";
				...
				$sms_sent=true;
				$LOG_FILE_HANDLER->logging('DEBUG'," $vcode Request has been taken We will send you Flipkart Gift card details on the requested Email id and Mobile Number($mobileno) and Email  MessageId:$MessageId and x_amzn_requestid:$x_amzn_requestid");
                        	// Send SMS
                	}
			$icici_voucher_update_str="";
			$icici_voucher_update_str.="voucher_status='requested',updated_at=now(),email='$email'";
			$icici_voucher_update_str.=",mobileno='$mobileno'";
			...
			if($commit==true)
				...
				$mysqlObj->rollback_transaction();
			return array("status"=>true,"successMessage"=>"Dear Customer your request has been registered. Your Flipkart Gift Voucher will be processed and sent to your registered Email Id and Mobile Number within the next 48 hours.");
		}	
                $voucher_detailsArr=$mysqlObj->getarray($select_query);
                if(!$voucher_detailsArr[0]['voucher_id'])
                {
                        $LOG_FILE_HANDLER->logging('ERROR',"Voucher Code not valid  : $select_query");
                        return array("status"=>false,"errorMessage"=>"Voucher Code is not valid");
                }
