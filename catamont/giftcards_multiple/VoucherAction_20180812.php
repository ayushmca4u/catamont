...
class VoucherAction
{
	...
	public function generate_voucher($request)
	{
		...
		if($voucher_status=="assigned" && ($dbmobileno!="" || $mobileno==""))
		{
			$LOG_FILE_HANDLER->logging('ERROR',"Flipkart Gifts Voucher Code with PIN Details already sent on your registered Email id or Mobile Number : $icici_select_query");
                        return array("status"=>false,"errorMessage"=>"Flipkart Gifts Voucher Code with PIN Details already sent on your registered Email id or Mobile Number");
		}
		...
		if($email && $byemail=="on" && $voucher_status=="created")
		{
			...
			$email_template.="<tr><td><img src='http://www.catamont.com/giftcards/flipkart.png'/></td></tr>";
...
13. In the event the beneficiary/Know Your Customer (KYC) details are found to be incorrect/ insufficient, QwikCilver Solutions/Flipkart.com retains the right to cancel the Gift Card issued.</br></td></tr>";
		...
		if($email_sent==true)	
		{
			...
			$icici_voucher_update_str.="voucher_status='assigned',updated_at=now(),voucher_assigned_date=now(),email='$email'";
		}
		if($sms_sent==true)
		{
			if($email_sent==true)
